...
\end{equation}
for $i=1,2,\dotsc,n$, with the following lower boundary condition
\begin{equation}
...
\end{equation}
for all, except possibly one, $i$ between $1$ and $n$. The boundary conditions were derived and characterized by Lebrun~\cite{Lebrun2006} (cf.~Proposition C.5 in~\cite{Lebrun2006}), but for completeness we will describe them below.

Let network operator $1$ be characterized by the smallest upper extremity, and network operator $2$ by the second smallest upper extremity; that is, let $\bar{\hat{c}}_1 < \bar{\hat{c}}_2\leq \bar{\hat{c}}_i$ for all $i\geq 2$. The upper boundary condition in Equation~\eqref{eq:foc_ode_upper_boundary_indirect} depends on the value of the upper bound on bids which is defined as follows (cf.~Definition~1 in~\cite{Lebrun2006})
\begin{equation}
...
Note that $\bar{\hat{c}}_1 < \bar{\hat{c}}_2\leq \bar{\hat{c}}_i$ for all $i\geq 2$ implies $\underline{\hat{c}}_1 < \underline{\hat{c}}_2\leq \bar{\hat{c}}_i$ for all $i\geq 2$. Let $J$ be the subset of $N$ such that (cf.~Definition~2 in~\cite{Lebrun2006})
\begin{equation*}
  J = \left\{j \:\middle\vert\: 1\leq j\leq n \text{ and } \underline{\hat{c}}_j < \bar{\hat{b}}\right\}.
\end{equation*}
$\hat{c}(\underline{\hat{b}})$ is defined as follows (cf.~Definition~3 in~\cite{Lebrun2006})
\begin{equation}
...
\end{equation}
for all $\underline{\hat{b}}\in (\underline{\hat{c}}_2, \bar{\hat{b}})$, where $k(\underline{\hat{b}})\in\{2,\dotsc,n\}$ such that $\underline{\hat{c}}_{k(\underline{\hat{b}})} < \underline{\hat{b}}$ and
\begin{equation*}
  \frac{1}{k(\underline{\hat{b}}) - 1}\sum_{i=1}^{k(\underline{\hat{b}})}\frac{1}{\underline{\hat{b}} - \underline{\hat{c}}_i} \leq\frac{1}{\underline{\hat{b}} - \underline{\hat{c}}_{k(\underline{\hat{b}})}},
\end{equation*}
and if $\underline{\hat{b}} < \underline{\hat{c}}_{k(\underline{\hat{b}})+1}$
\begin{equation*}
  \frac{1}{\underline{\hat{b}} - \underline{\hat{c}}_{k(\underline{\hat{b}})+1}}\leq\frac{1}{k(\underline{\hat{b}}) - 1}\sum_{i=1}^{k(\underline{\hat{b}})}\frac{1}{\underline{\hat{b}} - \underline{\hat{c}}_i}.
\end{equation*}

Recapping, Lebrun~\cite{Lebrun2006} showed that there exists a unique $\underline{\hat{b}}\in (\underline{\hat{c}}_2, \bar{\hat{b}})$, and hence, there exists a unique solution to the system of ODEs defined in Equation~\eqref{eq:foc_ode_indirect} with boundary conditions defined in Equations~\eqref{eq:foc_ode_lower_boundary_indirect}~and~\eqref{eq:foc_ode_upper_boundary_indirect}. However, the establishment of a closed-form solution to the system of ODEs in a generic setting ($n\ge 2$ network operators and arbitrary cost distribution) is very difficult (if even possible) \cite{Lebrun2006, Krishna10}.
...
As pointed out in Section~\ref{sec:generic_case_indirect}, the system of ODEs~\eqref{eq:foc_ode_indirect} together with the lower and upper boundary conditions~\eqref{eq:foc_ode_lower_boundary_indirect}~and~\eqref{eq:foc_ode_upper_boundary_indirect} does not possess any known closed-form solution. However, there exists a considerable research base studying methods for numerical approximation of the solution to the system of ODEs in question. See Hubbard and Paarsch~\cite{HubbardPaarsch2011} for an excellent overview of the subject.

The literature is mostly concerned with asymmetric first-price auctions in which the bidders are characterized by different probability distributions sharing a common support; that is, $F_i\neq F_j$ for at least one $i\in N$ such that $i\neq j, j\in N$ with $[\underline{\hat{c}}_i, \bar{\hat{c}}_i] = [\underline{\hat{c}}, \bar{\hat{c}}]$ for all $i\in N$. This is not true in our case. Therefore, the methods described in Hubbard and Paarsch~\cite{HubbardPaarsch2011} have to be adapted before they can be applied to our problem.

However, both the problems considered in the literature and the problem at hand share a common difficulty: the fact that the common lower bound on bids, $\underline{\hat{b}}$, is unknown \emph{a priori} \cite{HubbardPaarsch2011}. On the other hand, since the common upper bound on bids, $\bar{\hat{b}}$, is known \emph{a priori}, it would seem that any well-known finite-difference method, such as Euler or Runge-Kutta methods, could be applied with the upper bound on bids as a starting point (the so-called \emph{terminal value problem}). As shown by Hubbard and Paarsch~\cite{HubbardPaarsch2011}, the system~\eqref{eq:foc_ode_indirect} does not satisfy the Lipschitz condition for continuity as we approach the value of $\bar{\hat{b}}$. Therefore, much of the theory of ordinary differential equations no longer applies. In practice, this effectively means that the numerical solution obtained using a finite-difference method applied to the terminal value problem will quickly diverge. FIX:ME Provide a graphical example.

In this section, we shall consider two numerical algorithms which overcome the aforementioned problem: the forward shooting method, and the polynomial projection method, both of which were first proposed by Bajari~\cite{Bajari2001a}. To this end, let costs, $c_i$, be drawn from a uniform distribution for all network operators as in Section~\ref{sec:restricted_case_n_2_indirect}. Again, this implies that the distribution of costs-hats, $\hat{c}_i$, is uniform with supports $[\underline{\hat{c}}_i, \bar{\hat{c}}_i] = [(1-w)r_i, (1-w)r_i + w]$ for all $i\in N$. Furthermore, we shall concentrate only on cases in which $J=N$; that is, $\underline{\hat{c}}_i < \bar{\hat{b}}$ for all $i\in N$.
