...
\begin{equation*}
  \underline{\hat{c}}_i \leq \hat{c}(\underline{\hat{b}}) \quad\text{for all } i\in J.
\end{equation*}
Note, however, that this assumption cannot be enforced \emph{a priori} since $\underline{\hat{b}}$ is unknown. We can maximize the probability of enforcing this condition reasoning as follows. If $k(\underline{\hat{b}})=n$, then, by Definition~\ref{def:lower_bound_bids_indirect}, $\underline{\hat{c}}_n\leq \hat{c}(\underline{\hat{b}})$ and $\underline{\hat{c}}_n < \underline{\hat{b}}$. Furthermore, since $\underline{\hat{b}}\in (\underline{\hat{c}}_2, \bar{\hat{b}})$, then as the distance $(\underline{\hat{c}}_n-\underline{\hat{c}}_2)\to 0$, $k(\underline{\hat{b}})\to n$. Let $d(w,\epsilon) = (1-w)\epsilon$ where $\epsilon = r_n-r_2$. It can easily be verified that $d(w,\epsilon)\equiv (\underline{\hat{c}}_n-\underline{\hat{c}}_2)$. Since we already require $w\in (0.5,1)$, and since $\epsilon\in(0,1]$, then $d(w,\epsilon)\in(0,0.5)$ with $d(w,0)=0$. Thus, the probability that $\underline{\hat{b}}\in (\underline{\hat{c}}_n, \bar{\hat{b}})$ can be quantified as follows
\begin{align*}
  P\left\{ \underline{\hat{b}}\in (\underline{\hat{c}}_n, \bar{\hat{b}}) \right\} &= 1 - P\left\{\underline{\hat{b}}\in(\underline{\hat{c}}_2, \underline{\hat{c}}_n)\right\} = 1 - \frac{d(w,\epsilon)}{\bar{\hat{b}}-\underline{\hat{c}}_2}\\
  &\geq 1 - \frac{0.5}{\bar{\hat{b}} - \underline{\hat{c}}_2} \geq 1 - \frac{0.5}{\bar{\hat{c}}_1 - \underline{\hat{c}}_2}\\
  &= 1 - \frac{0.5}{(1-w)(r_1-r_2) + w}.
\end{align*}
