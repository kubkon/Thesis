...
\subsection{Numerical Stability of EFSM and CMA Methods} % (fold)
\label{sub:numerical_stability_of_efsm_and_cma_methods}
The following observation about the numerical stability of EFSM and CMA methods is given without proof. It has been observed during experimentation with the EFSM and CMA methods that, as the number of network operators increases and the value of price weight approaches $0.5$, the numerical stability of both methods is put into question. More specifically, for more than 4 network operators considered and price weight in the region $w\in(0.5,0.6)$, the methods might not converge on a valid solution. Therefore, it is crucial to test the generated approximations for the sufficiency condition of the Bayesian Nash equilibrium.

The most likely explanation is that this is due to the nature of the problem and the fact the problem does not satisfy the Lipschitz condition for continuity. This causes both methods to incorrectly settle on a result that is not the solution to the problem. While the FSM method also suffers from this issue as discussed in~\cite{HubbardPaarsch2011}, the complexity of the EFSM/CMA methods and its multistage operation makes it even more sensitive to the issue.
% subsection numerical_instability_of_efsm_and_cma_methods (end)
