...
In the generic case, with arbitrary probability distributions of costs and $n\ge 2$ network operators, recall that: if $w=0$, then Proposition~\ref{prop:special_case_w_0_direct} holds; if $w=1$, then Proposition~\ref{prop:special_case_w_1_direct} holds; and if $r_i=r_j$ for all $i,j\in N$ such that $i\neq j$, then Corollary~\ref{cor:special_case_r_i_r_j_direct} holds. Thus, we assume the following.
\begin{assumptions}
\label{ass:assumptions_generic_indirect}$\quad$\\
\begin{enumerate}
  \item Suppose $w\in(0,1)$.
  \item Assume that there exists only one network operator characterized by the lowest reputation rating. Without loss of generality, let network operator 1 be characterized by the lowest reputation rating. Furthermore, let network operator 2 be characterized by the second lowest reputation rating, and so on; that is, $r_1 < r_2 \leq \cdots\leq r_n$. Note that, this implies $\underline{\hat{c}}_1 < \underline{\hat{c}}_2 \leq \cdots \leq\underline{\hat{c}}_n$ and $\bar{\hat{c}}_1 < \bar{\hat{c}}_2 \leq\cdots \leq \bar{\hat{c}}_n$.
\end{enumerate}
\end{assumptions}
...
\end{equation}
for all, except possibly one, $i\in N$.
\end{proposition}

Even though we are guaranteed that there exists a unique equilibrium to our problem, the establishment of a closed-form solution to the system of ODEs in a generic setting ($n\ge 2$ network operators and arbitrary cost distribution) is very difficult (if even possible) \cite{Lebrun2006, Krishna10}.
