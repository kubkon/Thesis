...
\begin{figure}[t]
  ...
  \caption{FIX:ME Lipschitz violation}
...
In this subsection, we present the approximation results for two bidding scenarios using both algorithms: the forwards shooting method and the polynomial projection method. The implementation details of the algorithms together with the instructions on how to us the Haskell scripts developed during this research work can be found in Appendix~\ref{cha:haskell_numerical_tools}.
...
In the first examined scenario, there are 3 network operators characterized by reputation ratings as summarized in Table~\ref{tab:approximation_scenario_3_indirect}. Furthermore, the price weight is set to $0.75$. Figures~\ref{fig:forward_shooting_3_indirect}~and~\ref{fig:forward_shooting_3_sufficiency_indirect} depict the results of the approximation generated by the forward shooting method, while Figures~\ref{fig:polynomial_projection_3_indirect}~and~\ref{fig:polynomial_projection_3_sufficiency_indirect} depict the results of the approximation generated by the polynomial projection method. Both approximation methods yield virtually the same estimate of the lower bound on bids, $\underline{\hat{b}}\approx 0.375$. In the forward shooting method case, the approximation flattens out as $b$ approaches $\bar{\hat{b}}$, and therefore, the approximation satisfies the sufficiency only until $b$ reaches a close neighborhood of $\bar{\hat{b}}$. This is due to the fact that the system of ODEs does not satisfy Lipschitz condition for continuity as $b$ approaches $\bar{\hat{b}}$. The polynomial projection method, on the other hand, eliminates this problem entirely, and the approximation satisfies sufficiency for all $b\in[\underline{\hat{b}},\bar{\hat{b}}]$.
...
In the second examined scenario, there are 4 network operators characterized by reputation ratings as summarized in Table~\ref{tab:approximation_scenario_4_indirect}. Furthermore, the price weight is set to $0.85$. Figures~\ref{fig:forward_shooting_4_indirect}~and~\ref{fig:forward_shooting_4_sufficiency_indirect} depict the results of the approximation generated by the forward shooting method, while Figures~\ref{fig:polynomial_projection_4_indirect}~and~\ref{fig:polynomial_projection_4_sufficiency_indirect}. Both approximation methods yield virtually the same estimate of the lower bounds on bids, $\underline{\hat{b}}\approx 0.38$. Similarly to the previous bidding scenario with 3 network operators, in the forward shooting method case, the approximation flattens out as $b$ approaches $\bar{\hat{b}}$, and therefore, the approximation satisfies the sufficiency only until $b$ reaches a close neighbourhood of $\bar{\hat{b}}$. In the polynomial projection method case, the approximation satisfies sufficiency for all $b\in[\underline{\hat{b}},\bar{\hat{b}}]$.
...
\end{equation*}
for all $i\in N$, and hence consider all nontrivial equilibria characterized by Lebrun (see Proposition~\ref{prop:characterization_of_the_equilibrium_indirect}). Note, further, that all the remaining assumptions of Section~\ref{sec:numerical_analysis_indirect} hold. To be more specific, we shall concentrate on cases such that $w\in (0.5,1.0)$ for all $r_i\in[0,1], i\in N$ such that $r_1\leq\cdots\leq r_n$ with at least one inequality strict.
...
The extended forward shooting method extends the forward shooting method by effectively implementing the reasoning behind the bidding extension characterized by Lebrun~\cite{Lebrun2006} (see Section~\ref{sec:numerical_analysis_indirect} for a description of the extension). As such, like the original method, the extended forward shooting method aims at finding the best approximation of the lower bound on bids, $\underline{\hat{b}}'$ say, by successively picking a value from the feasible interval $(\underline{\hat{c}}_2,\bar{\hat{b}})$, and verifying whether a numerical solution to the initial value problem
\begin{equation}
...
\Ensure{Approximation to $\underline{\hat{b}}$}
  ...
  \Function{extForwardShooting}{$low, high$}
    ...
    \Else
      ...
      \StatexIndent[8.5]{evaluated at points $b\in bids$, and $k(\underline{\hat{b}}) = k$}
      \StatexIndent[8.5]{and $\hat{c}(\underline{\hat{b}}) = \hat{c}$}
      \If{$(bids,costs_i)\in S_i$ for all $i\gets 1$ to $n$}
...
The pseudo-code for the extended forward shooting method is shown in listing Algorithm~\ref{alg:extended_forward_shooting_method_indirect}. The flow of the algorithm is almost exactly the same as for the forward shooting method. The main differences are twofold: the algorithm estimates $k(\underline{\hat{b}}')$ and $\hat{c}(\underline{\hat{b}}')$ (line 6); and the algorithm solves the system~\eqref{eq:efsm_initial_value_problem_indirect} according to a customized finite-differences method described below (line 8).

Listing Algorithm~\ref{alg:function_estimate_k_c_indirect} depicts the pseudo-code for the function estimateKC which estimates $k(\underline{\hat{b}}')$ and $\hat{c}(\underline{\hat{b}}')$. It takes as an input an estimate of the lower bound on bid, $\underline{\hat{b}}'$, and the set of lower extremities, $\underline{\hat{c}}_i$ for all $i\in N$. The function then iterates over $k\in\{2,\ldots,n\}$, and for each $k$ it computes $\hat{c}(\underline{\hat{b}}')$ according to Equation~\eqref{eq:lower_bound_bids_cost_indirect}. If $k<n$ and $\hat{c}(\underline{\hat{b}}')$ satisfies~\eqref{eq:bounds_lower_bound_bids_cost_indirect}, then the function returns that particular pair of values $(k, \hat{c}(\underline{\hat{b}}'))$ such that $2\leq k < n$. Otherwise, $k=n$ is returned which reduces system~\eqref{eq:efsm_initial_value_problem_indirect} to~\eqref{eq:fsm_initial_value_problem_indirect}, and the extended forward shooting method reduces to the forward shooting method.

\begin{figure}[t]
  \includegraphics[width=\figsize]{Indirect/Figures/lebrun_solution}
  \caption{FIX:ME Explanation of the Lebrun's proof and the numerical approximation procedure}
...
\item Since $\underline{\hat{c}}_1 < \underline{\hat{c}}_2 < \hat{c}(\underline{\hat{b}}) < \underline{\hat{c}}_3 < \underline{\hat{c}}_4$, then $k(\underline{\hat{b}}) = 2$, and hence, network operators 1 and 2 solve
\begin{equation}
...
\item Next, $k(\underline{\hat{b}})$ is incremented by $1$ over the previous value, and the procedure in 1) is repeated with this difference that now network operators 1, 2 and 3 solve system~\eqref{eq:efsm_lebrun_ode_1_indirect} while network operator 4 solves~\eqref{eq:efsm_lebrun_ext_1_indirect}. The system~\eqref{eq:efsm_lebrun_ode_1_indirect} is solved for $b\in [b(\underline{\hat{c}}_3), \bar{\hat{b}}]$, but only bid values such that $b(\underline{\hat{c}}_3)\leq b\leq b(\underline{\hat{b}}_4)$ are kept (Stage 2, Figure~\ref{fig:lebrun_solution_indirect}).

\item Finally, all network operators solve the system~\eqref{eq:efsm_lebrun_ode_1_indirect} for bid values $b\in [b(\underline{\hat{c}}_4),\bar{\hat{b}}]$ (Stage 3, Figure~\ref{fig:lebrun_solution_indirect}).
\end{enumerate}
...
Furthermore, similarly to the implementation of forward shooting and polynomial projection methods, we assumed that
\begin{equation*}
  F_i(x) = \frac{x - \underline{\hat{c}}_i}{\bar{\hat{c}}_i - \underline{\hat{c}}_i} \quad\text{and}\quad f_i(x) = \frac{1}{\bar{\hat{c}}_i - \underline{\hat{c}}_i}
\end{equation*}
...
The combined methods approach is equivalent to the extended forward shooting method with one difference: the last stage in the customized finite-difference method is solved using the polynomial projection method instead of the standard forward shooting method (Stage 3 in Figure~\ref{fig:lebrun_solution_indirect}). The polynomial projection method should reduce the error in the forward shooting method approximation as $b$ approaches $\bar{\hat{b}}$ and the system of ODEs does not satisfy the Lipschitz condition for continuity.
% subsection combined_methods_approach_indirect (end)

\subsection{Approximation Results} % (fold)
\label{sub:approximation_results_indirect}
Similarly to the algorithms presented in Section~\ref{sec:numerical_analysis_indirect}, both the extended forward shooting method and combined methods approach were tested for correct implementation using the same procedure as presented in Section~\ref{sec:numerical_analysis_indirect}. Therefore, for brevity, we omit the discussion of the verification results in this section.

In the first examined scenario, there are 3 network operators characterized by reputation ratings as summarized in Table~\ref{tab:approximation_scenario_ext_3_indirect}. Furthermore, the price weight is set to $0.55$. Figures~\ref{fig:efs_3_indirect}~and~\ref{fig:efs_3_sufficiency_indirect} depict the results of the approximation generated by the extended forward shooting method, while Figures~\ref{fig:combined_3_indirect}~and~\ref{fig:combined_3_sufficiency_indirect} depict the results of the approximation generated by the combined methods approach. Since the combined methods approach is an improvement over the extended forward shooting method for $b$ in the close neighbourhood of $\bar{\hat{b}}$, both methods yield the same estimate of the lower bound on bids, $\underline{\hat{b}}\approx 0.41$. Furthermore, the approximations generated by both methods satisfy the sufficiency for all $b\in[\underline{\hat{b}},\bar{\hat{b}}]$.
...
\begin{figure}[p!]
  ...
  \caption{FIX:ME Extended forward shooting: $w=0.55, r_1 = 0.25, r_2 = 0.5, r_3 = 0.75$}
...
\begin{figure}[p!]
  ...
  \caption{FIX:ME Combined: $w=0.55, r_1 = 0.25, r_2 = 0.5, r_3 = 0.75$}
...
In the second examined scenario, there are 4 network operators characterized by reputation ratings as summarized in Table~\ref{tab:approximation_scenario_ext_4_indirect}. Furthermore, the price weight is set to $0.55$. Figures~\ref{fig:efs_4_indirect}~and~\ref{fig:efs_4_sufficiency_indirect} depict the results for the approximation generated by the extended forward shooting method, while Figures~\ref{fig:combined_4_indirect}~and~\ref{fig:combined_4_sufficiency_indirect} depict the results of the approximation generated by the combined methods approach. In both cases, the estimate of the lower bound on bids is $\underline{\hat{b}}\approx 0.365$. In the extended forward shooting method case, the approximation satisfies the sufficiency only until $b$ reaches a close neighborhood of $\bar{\hat{b}}$, which again is due to the fact that the system of ODEs does not satisfy the Lipschitz condition for continuity as $b$ approaches $\bar{\hat{b}}$. The combined methods approach, on the other hand, eliminates this problem since it uses the polynomial projection method to appromixate the solution in the close neighborhood of $\bar{\hat{b}}$ (cf.~Stage 3 in Figure~\ref{fig:lebrun_solution_indirect}). Nevertheless, even in the combined methods approach, the approximation satisfies the sufficiency only approximately in that neighborhood. The most likely explanation is the fact that
...
\begin{figure}[p!]
  ...
  \caption{FIX:ME Extended forward shooting: $w = 0.55, r_1 = 0.2, r_2 = 0.4, r_3 = 0.6, r_4 = 0.8$}
...
\begin{figure}[p!]
  ...
  \caption{FIX:ME Combined: $w = 0.55, r_1 = 0.2, r_2 = 0.4, r_3 = 0.6, r_4 = 0.8$}
...
\subsection{Comparison of Direct and Indirect Approaches} % (fold)
