...
\Ensure{Approximate $\underline{\hat{b}}$; $\alpha_i\in\mathbb{R}^K$ for all $i\in N$}
  ...
  \Repeat
    \Let{$bids$}{$B$}
    \Let{$(\underline{b}, \alpha_1, \dotsc, \alpha_n)$}{minimize~\eqref{eq:least_squares_objective_function_indirect} with initial values $(\underline{b}, \alpha_1, \dotsc, \alpha_n)$}
...
In this subsection, we present the approximation results for two bidding scenarios using both algorithms: the forward shooting method and the polynomial projection method. The implementation details of the algorithms together with the instructions on how to us the Cython scripts developed during this research work can be found in Appendix~\ref{cha:numerical_software}.
...
Furthermore, similarly to the implementation of forward shooting and polynomial projection methods, in the implementation of extended forward shooting method, we assumed that
\begin{equation*}
  F_i(x) = \frac{x - \underline{\hat{c}}_i}{\bar{\hat{c}}_i - \underline{\hat{c}}_i} \quad\text{and}\quad f_i(x) = \frac{1}{\bar{\hat{c}}_i - \underline{\hat{c}}_i}
\end{equation*}
...
The combined methods approach is equivalent to the extended forward shooting method with one difference: the last stage in the customized finite-difference method is solved using the polynomial projection method instead of the standard forward shooting method (Stage 3 in Figure~\ref{fig:lebrun_solution_indirect}). The polynomial projection method should reduce the error in the approximation generated by the forward shooting method as $b$ approaches $\bar{\hat{b}}$, where the system of ODEs does not satisfy the Lipschitz condition for continuity.
% subsection combined_methods_approach_indirect (end)

\subsection{Approximation Results} % (fold)
\label{sub:approximation_results_indirect}
In this subsection, we present the approximation results for two bidding scenarios using both algorithms: the extended forward shooting method and the combined methods approach. The implementation details of the algorithms together with the instructions on how to us the Cython scripts developed during this research work can be found in Appendix~\ref{cha:numerical_software}.
...
The aim of this section is threefold. First, the results from direct and indirect approaches are compared. Second, the prices the subscriber has to pay for each value of the price weight given a tuple of network operators' reputation ratings (in the restricted case) are examined. Finally, other possible numerical methods are summarized.
...
\subsection{Other Numerical Methods} % (fold)
\label{sub:other_numerical_methods}

% subsection other_numerical_methods (end)
