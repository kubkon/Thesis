...
\end{equation}
\annotate{}{for the price weight $w$. In this way, the subscriber is guaranteed the expected price of at most $p_1$.}
...
\end{equation}
\annotate{}{2) it is monotonically increasing everywhere except possibly at $\bar{\hat{b}}$, that is,}
\begin{equation}
  \label{eq:fsm_condition_2_indirect}
  b_1 < b_2\implies s_i(b_1) < s_i(b_2) \textrm{ for all }b_1,b_2\in [\underline{\hat{b}}', \bar{\hat{b}});
\end{equation}
\annotate{}{and 3) each function value is strictly lower than its argument except possibly at $\bar{\hat{b}}$, that is,}
\begin{equation}
