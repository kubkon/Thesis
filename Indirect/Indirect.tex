...
\begin{equation*}
  \underline{\hat{c}}_i \leq \hat{c}(\underline{\hat{b}}) \quad\text{for all } i\in J.
\end{equation*}
Note, however, that this assumption cannot be enforced \emph{a priori} since $\underline{\hat{b}}$ is unknown. We postulate (without proof) that the assumption should be satisfied for $w\in[0.6, 1.0]$ and $r_i\in[0.5, 1.0]$ for all $i\in J$.
