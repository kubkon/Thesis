...
The methods discussed in this chapter could be further improved upon. In particular, the PPM method would potentially yield more accurate results if Chebyshev polynomials were employed in place of regular polynomials since they are better suited to least squares optimization~\cite{Hubbard2011,MasonApproximation2003}. That is, the inverse equilibrium bidding function in Equation~\eqref{eq:polynomial_cost_function_indirect} can be approximated using Chebyshev polynomials as follows
\begin{equation*}
...
\end{align*}
and $x(b;\underline{\hat{b}})\in [-1,1]$ for all $b\in [\underline{\hat{b}}, \bar{\hat{b}}]$. The set $B$ consists of $T = |B|$ points such that
\begin{equation*}
...
\end{equation*}
The grid generated this way has the property of minimizing the interpolation error when approximating a function.
