...
\begin{equation*}
  u_i(\hat{b}_i, \hat{c}_i) = \frac{1}{w}\cdot \hat{u}_i(\hat{b},\hat{c}).
\end{equation*}
\end{proposition}
...
Let
\begin{equation*}
  \displaystyle\hat\Pi_i(\hat{b}_i,\hat{c}_i,\hat{b}_{-i},\hat{c}_{-i}) = \hat{u}_i(\hat{b}, \hat{c})P\{\textrm{winning}\:|\:\hat{b}_i\}
\end{equation*}
and
\begin{equation*}
  \displaystyle\Pi_i(\hat{b}_i,\hat{c}_i,\hat{b}_{-i},\hat{c}_{-i}) = u_i(\hat{b}, \hat{c})P\{\textrm{winning}\:|\:\hat{b}_i\}.
\end{equation*}
$\hat\Pi_i$ and $\Pi_i$ are therefore expected value of the utility functions $\hat{u}_i$ and $u_i$ respectively. By definition of the pure-strategy Bayesian Nash equilibrium~\cite{Gibbons92}, for all $i$ and $\hat{c}_i$,
\begin{equation*}
  \hat\Pi_i(\hat{b}_i^*,\hat{c}_i,\hat{b}_{-i}^*,\hat{c}_{-i})\geq \hat\Pi_i(\hat{b}_i,\hat{c}_i,\hat{b}_{-i}^*,\hat{c}_{-i})
\end{equation*}
for all $\hat{b}_i$. Multiplying both sides of the inequality by $\frac{1}{w} > 0$ yields, for all $i$ and $\hat{c}_i$
\begin{align*}
