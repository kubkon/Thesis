...
In the rest of this thesis, we concentrate on the utility function with the scaling factor, $\displaystyle\sfrac{1}{w}$, omitted; that is, let
\begin{equation}
...
With those definitions at hand, we can proceed with the characterisation of the equilibrium which is due to Lebrun~\cite{Lebrun2006}.
...
\begin{enumerate}
  ...
  \begin{enumerate}
    \item Calculate the common bids-hat support $[\underline{\hat{b}}, \bar{\hat{b}}]$ (see Equation~\eqref{eq:bounds_bid_restricted_indirect}).
...
Furthermore, in the implementation of the FSM, we assumed that
\begin{equation}
...
In the PPM, we assume that the inverse equilibrium bidding function for each network operator $i\in N$ can be approximated by a $K^{\textrm{th}}$ order polynomial of the form
\begin{equation}
...
\end{equation}
where $K\in\mathbb{N}_+$ and $K\geq 2$, and $\alpha_i = (\alpha_{i,1},\dotsc,\alpha_{i,K})^T$ is a $K$-tuple of unknown polynomial coefficients such that $\alpha_i\in\mathbb{R}^{K}$. The idea behind the method is then to employ a nonlinear optimisation technique, such as the Nelder-Mead method, to find an approximation to the lower bound on bids and the set of polynomial coefficients that best satisfy the system of ODEs~\eqref{eq:foc_ode_indirect} with lower and upper boundary conditions~\eqref{eq:foc_ode_lower_boundary_indirect} and \eqref{eq:foc_ode_upper_boundary_indirect} in the least squares sense. That is, minimise the least squares objective function
\begin{equation}
...
Furthermore, in the implementation of the PPM, we made the same simplification as in the implementation of the forward shooting method; that is,
\begin{equation}
  F_i(x) = \frac{x - \underline{\hat{c}}_i}{\bar{\hat{c}}_i - \underline{\hat{c}}_i} \quad\textrm{and}\quad f_i(x) = \frac{1}{\bar{\hat{c}}_i - \underline{\hat{c}}_i}
\end{equation}
...
\end{align}
and it is there mainly to avoid possible divisions by zero which are not handled properly by the GSL library. It is important to realise, however, that by enforcing this assumption, we implicitly assume that the algorithm always operates in the feasible region; that is,
\begin{equation}
...
\begin{figure}[p!]
  ...
  \caption{FSM solution to the bidding problem characterised by: $w=0.75$, $r_1 = 0.25$, $r_2 = 0.5$, and $r_3 = 0.75$}
...
\begin{figure}[p!]
  ...
  \caption{PPM solution to the bidding problem characterised by: $w=0.75$, $r_1 = 0.25$, $r_2 = 0.5$, and $r_3 = 0.75$}
...
\begin{figure}[p!]
  ...
  \caption{FSM solution to the bidding problem characterised by: $w=0.85$, $r_1 = 0.2$, $r_2 = 0.4$, $r_3 = 0.6$, and $r_4=0.8$}
...
\begin{figure}[p!]
  ...
  \caption{PPM solution to the bidding problem characterised by: $w=0.85$, $r_1 = 0.2$, $r_2 = 0.4$, $r_3 = 0.6$, and $r_4=0.8$}
...
\subsection{Improved Polynomial Projection Method using Chebyshev Polynomials} % (fold)
\label{sub:improved_polynomial_projection_method_using_chebyshev_polynomials_indirect}
The PPM method could potentially be improved in the sense that it would yield more accurate results if Chebyshev polynomials were employed in place of regular polynomials. Chebyshev polynomials are better suited to least squares optimisation upon which the PPM method is based~\cite{MasonApproximation2003,Hubbard2011}.

In order to facilitate the improvement, the inverse equilibrium bidding function in Equation~\eqref{eq:polynomial_cost_function_indirect} can be approximated using Chebyshev polynomials as follows
\begin{equation}
...
\end{equation}
Generated in this way, the grid of points, more commonly known as Chebyshev nodes, is said to minimise the interpolation error when approximating a function~\cite{Hubbard2011}.

The cost functions defined this way should potentially yield more accurate as well as stable numerically results, especially if the Mathematical Programming with Equilibrium Constraints (MPEC) optimisation approach is employed~\cite{HubbardPaarsch2011,Hubbard2011}.
...
In what follows, we present a numerical algorithm which improves upon the algorithms considered in Section~\ref{sec:numerical_analysis_indirect}: the extended forward shooting method (EFSM). Furthermore, we analyse the numerically approximated equilibrium bidding strategies for two bidding scenarios generated using the algorithm.
...
\begin{figure}[p!]
  ...
  \caption{EFSM solution to the bidding problem characterised by: $w=0.55$, $r_1 = 0.25$, $r_2 = 0.5$, and $r_3 = 0.75$}
...
\begin{figure}[p!]
  ...
  \caption{EFSM solution to the bidding problem characterised by: $w=0.55$, $r_1 = 0.2$, $r_2 = 0.4$, $r_3 = 0.6$, and $r_4 = 0.8$}
