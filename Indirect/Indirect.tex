...
Let network operator $1$ be characterized by the smallest upper extremity, and network operator $2$ be characterized by the second smallest upper extremity; that is, let $\bar{\hat{c}}_1 < \bar{\hat{c}}_2\leq \bar{\hat{c}}_i$ for all $i\geq 2$. The upper boundary condition in Equation~\eqref{eq:foc_ode_upper_boundary_indirect} depends on the value of the upper bound on bids which is defined as follows (cf.~Definition~1 in~\cite{Lebrun2006})
\begin{equation}
...
\section{Numerical Solution} % (fold)
\label{sec:numerical_solution}

% section numerical_solution (end)
