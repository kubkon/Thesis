...
\end{equation*}
Let $n' = |J|$. Then, $\hat{c}(\underline{\hat{b}})$ is defined as follows (cf.~Definition~3 in~\cite{Lebrun2006})
\begin{equation}
...
\end{equation}
for all $\underline{\hat{b}}\in (\underline{\hat{c}}_2, \bar{\hat{b}})$, where there exists one and only one $k(\underline{\hat{b}})\in\{2,\dotsc,n'\}$ (cf.~Lemma~A4.1 in Lebrun~\cite{Lebrun2004}) such that $\underline{\hat{c}}_{k(\underline{\hat{b}})} < \underline{\hat{b}}$ and
\begin{equation*}
...
\end{equation*}
Note that the two inequalities together with Equation~\eqref{eq:lower_bound_bids_cost_indirect} imply
\begin{equation}
