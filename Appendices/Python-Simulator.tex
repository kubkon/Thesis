...
The simulator is composed of four building blocks: \lstinline{SimulationEngine} class, \lstinline{EventHandler} class, \lstinline{Event} class, and \lstinline{PRNG} class. \lstinline{SimulationEngine} class provides all the core functionality of a DES simulator; that is, it advances the simulation time, and implements the event list. \lstinline{EventHandler} class is an abstract base class (or an interface), and provides an outline of the basic functionality that any proper subclass must implement. In short, it is responsible for generating and handling simulation specific events; for example, customer arrival and departure events in a simulation of an M/M/1 queue. \lstinline{Event} class implements the basic structure of an event used by the simulator. Finally, \lstinline{PRNG} class provides basic pseudo-random number generation routines; i.e., it is a wrapper class for the Py3k `random' module.

\begin{figure}[t]
  \includegraphics[width=3in]{Appendices/Figures/simulation_flow}
  \caption{The simulation flow}
  \label{fig:simulation_flow_simappendix}
\end{figure}

Figure~\ref{fig:simulation_flow_simappendix} depicts the basic flow of the simulation. Upon the start of the simulation, the \lstinline{SimulationEngine} instance notifies the \lstinline{EventHandler} instance\footnote{Technically, the \lstinline{SimulationEngine} instance notifies an instance of a class that subclasses \lstinline{EventHandler} class; however, to keep the description simple and generic, we shall refer to it as \lstinline{EventHandler} instance.} of the start of the simulation (via a callback mechanism). At this point, the \lstinline{EventHandler} instance should handle it by generating the first event, and passing it to the \lstinline{SimulationEngine} for scheduling. Afterwards, the \lstinline{SimulationEngine} dequeues the next event from the event list, and passes it to the \lstinline{EventHandler}, which should update statistics related to the passed in event, and generate additional events. The process is repeated until there are no events in the event list. When the list becomes empty, the \lstinline{SimulationEngine} notifies the \lstinline{EventHandler} of the stop of the simulation. At this point, the \lstinline{EventHandler} should save/finalize all the statistics relevant to the simulation for further processing and analysis.
