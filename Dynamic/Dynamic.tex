...
\chapter{Dynamic Analysis of Network Selection Mechanism} % (fold)
\label{cha:dynamic}

\minitoc
\vspace{10mm}

The results presented in Chapters~\ref{cha:direct}~and~\ref{cha:indirect} describe only a single stage of the selection process employed in the Digital Marketplace (DMP); i.e., a one-shot DMP auction. As such, the temporal (or dynamic) aspect of the process was neglected. The main reason for such a decision was as follows. Concentrating only on a single stage of the process considerably simplified the inherent analytical complexity of the problem. It rendered the analytical analysis tractable in case of two competing bidders, and allowed for the numerical analysis of scenarios with more than two network operators. As a result, light was shed on the appropriateness of the DMP auction as a network selection mechanism at least in case of two bidders. In other words, if the results of the analysis of the one-shot DMP auction were unfavorable, then that would already render the mechanism inappropriate, and hence, little would be gained from the inclusion of the temporal aspect in the analysis.
...
\begin{equation}
  ...
  \end{array}\right.
\end{equation}
where $\beta$ is defined as in Equation~\eqref{eq:def_beta_direct}; that is,
\begin{equation}
