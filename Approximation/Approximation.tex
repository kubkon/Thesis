...
In this section, we analyse the results for three bidding scenarios: with $n=2$, $n=3$ and $n=4$ bidders respectively. We concentrate on only up to four bidders since the time required to simulate the problem increases exponentially with each additional bidder. It should be noted that the simulations were run on a 12-core Xeon processor, and were fully parallelised (i.e., each repetition was run in a separate process). The time required to complete each simulation run took, on average: 2 hours to finish simulation for $n=2$ bidders, 8 hours for $n=3$ bidders, and 72 hours for $n=4$ bidders FIX:ME. Furthermore, we note that since the UK market is currently dominated by an oligopoly of four network operators (bidders) who own their infrastructure (EE, Vodafone, O2, and Three), solving the problem for four bidders is directly relevant.
