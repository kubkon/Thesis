...
Furthermore, it is worth noting that the mean relative errors in price and ex-ante expected utilities for all bidders is ``noisier'' from point to point than in the 2 bidders' case; i.e., unlike in case of 2 bidders, the points do not form a smooth relationship. It is not immediately obvious why such a difference; however, there are two possible explanations that are incidentally closely related. Firstly, as shown in Section~\ref{sec:extended_numerical_analysis_indirect}, in case of 3 or more bidders, the DMP requires the use of the EFSM algorithm to solve for the equilibrium bidding strategies. Since the reputation rating triples are randomly drawn per price weight, there exists a possibility that some reputation rating triples will lead to an extended scenario (cf.~Figure~\ref{fig:lebrun_solution_indirect}). Since the proportion of the triples leading to the extended scenario is random from price weight to price weight, this leads to discrepancies (``noise'') between the points. This brings us to the second explanation, namely, that 100 reputation rating triples per price weight may not be sufficient. Generating more than 100 triples, however, comes at a significant increase in the simulation time.
