...
In this section, we analyse the results for four bidding scenarios: with $n=2$, $n=3$, $n=4$ and $n=5$ bidders respectively. We concentrate on only up to five bidders since the time required to simulate the problem increases quadratically with each additional bidder. It should be noted that the simulations were run on a 12-core Xeon processor, and were fully parallelised (i.e., each repetition was run in a separate process, and up to 20 processes were running at any one time). The time required to complete each simulation run took approximately: 1.6 hours for $n=2$ bidders, 25.6 hours for $n=3$ bidders, 76.2 hours for $n=4$ bidders, and 153.4 hours for $n=5$ bidders. Figure~\ref{fig:simulation_time_approximation} depicts results of fitting a polynomial of the order two to the data. It is clear from the figure that simulating for more than five bidders takes a considerable amount of time, and is, therefore, impractical.

\begin{figure}[t!]
  \includegraphics[width=\figsize]{Approximation/Figures/simulation_time}
  \caption{Polynomial of the order two fitted to the simulation time data}
