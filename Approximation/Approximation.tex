...
The most straightforward way of comparing the approximation with the \emph{benchmark} (original, DMP) auction is to compare the expected profits (or utilities) for all bidders. This can be accomplished by applying the expected value operator to the utility function (Equation~\eqref{eq:utility_approximation}) for each bidder; that is, we define the expected utility as
\begin{equation}
...
It is important to note that expected utility is a function of cost. Thus, in order to quantify this comparison methodology, in our case, it is equivalent to measuring the difference (or distance/distortion) between two curves (two expected utility functions). This can be accomplished by adopting the Kolmogorov-Smirnov (K-S) statistic used in the statistical test for testing whether a random sample comes from a reference probability distribution, known as the Kolmogorov-Smirnov test. Recall that the K-S statistic is defined by
\begin{equation*}
  D = \sup_x |G(x) - F(x)|
\end{equation*}
where $F(x)$ is the cdf of the reference probability distribution, $G(x)$ is the cdf of the random sample, and $x$ comes from the (common) domain of both functions. For the purpose of our comparison, we shall replace the supremum with maximum, and cdf functions with expected utility functions from both auctions for the corresponding bidder; that is,
\begin{equation}
  \label{eq:k_s_statistic_approximation}
  \displaystyle D = \max_{c_i\in \mathscr{D}(\Pi_i^\textrm{DMP})} \left\vert\Pi_i^{\textrm{CP}}(c_i) - \Pi_i^{\textrm{DMP}}(c_i)\right\vert
\end{equation}
...
\begin{figure}[p!]
  \includegraphics[width=\figsize]{Approximation/Figures/test_dm_exp_utilities}
  \caption{FIX:ME}
  \label{fig:test_dm_exp_utilities_approximation}
  \vspace{10mm}
  \includegraphics[width=\figsize]{Approximation/Figures/test_common_priors_exp_utilities}
  \caption{FIX:ME}
  \label{fig:test_common_priors_exp_utilities_approximation}
\end{figure}
